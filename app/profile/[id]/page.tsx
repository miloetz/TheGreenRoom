...
export default function ProfileView() {
  ...
  const handleMessage = async () => {
    if (!currentUser || !profile || currentUser.user_type === profile.user_type) return

    setIsMessaging(true)
    try {
      // Determine who is the musician and who is the venue
      const musicianId = currentUser.user_type === 'musician' ? currentUser.id : profile.id
      const venueId = currentUser.user_type === 'venue' ? currentUser.id : profile.id
  ...
  // Can message if viewing another user's profile and user types are different
  const canMessage = currentUser && profile &&
    currentUser.id !== profile.id &&
    currentUser.user_type !== profile.user_type
